...
var Note = {
		...
		me.getNote(noteOrContent.NoteId, function(dbNote) {
			...
			// 更新
				...
				if(needUpdate) {
					...
					var isTrash = false;
					var markUpdateNum = false;
					if(dbNote['IsTrash']) {
						updates['IsTrash'] = isTrash;
						isDirty = true;
						// 需要更新数据
						markUpdateNum = true;						
					}
					...
					db.notes.update({NoteId: noteOrContent.NoteId}, { $set: updates }, {}, function (err, numReplaced) { 
						if(err) {
							...
							if(markUpdateNum) {
								//Web.Notebook.incrNotebookNumberNotes(dbNote['NotebookId']);
								noteOrContent['NotebookId'] = dbNote['NotebookId'];
								noteOrContent['markUpdateNum'] = true;
							}
	...
	_getNotes: function(notebookId, isTrash, isStar, callback) {
		...
		var query = {
			...
			NoteId: {$exists:true, $ne:null},// NoteId is not empty
		};
		if(isStar) {
			query['Star'] = true;
		}
	...
	searchNote: function(key, callback) {
		...
		var userId = User.getCurActiveUserId();
		db.notes.find({UserId: userId, IsTrash: false, NoteId: {$exists:true, $ne:null}, LocalIsDelete: false, $or: [{Title: reg}, {Content: reg}]}).sort({'UpdatedTime': -1}).exec(function(err, notes) {
			if(!err && notes) {
	...
	searchNoteByTag: function(tag, callback) {
		var userId = User.getCurActiveUserId();
		db.notes.find({UserId: userId, IsTrash: false, NoteId: {$exists:true, $ne:null}, LocalIsDelete: false, Tags: {$in: [tag]}}).sort({'UpdatedTime': -1}).exec(function(err, notes) {
			if(!err && notes) {
	...
	hasNotes: function(notebookId, callback) {
		db.notes.count({NotebookId: notebookId, NoteId: {$exists:true, $ne:null}, IsTrash: false, LocalIsDelete: false}, function(err, n) {
			console.log(n);
			if(err || n > 0) {
	...
	getDirtyNotes: function(callback) {
		var me = this;
		db.notes.find({UserId: User.getCurActiveUserId(), NoteId: {$exists:true, $ne:null}, IsDirty: true}, function(err, notes) {
			if(err) {
				log(err);
				return callback && callback(false);
			} else {
	...
	*/
	...
	countNoteByTag: function(title, callback) {
		var userId = User.getCurActiveUserId();
		db.notes.count({UserId: userId, LocalIsDelete: false , NoteId: {$exists:true, $ne:null}, Tags: {$in: [title]}}, function(err, cnt) {
	...
	updateNoteToDeleteTag: function(title, callback) {
		...
		db.notes.find({UserId: userId, LocalIsDelete: false , NoteId: {$exists:true, $ne:null}, Tags: {$in: [title]}}, function(err, notes) {
