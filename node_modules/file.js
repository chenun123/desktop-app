...
var File = {
	...
	getFileContent: function(filePath) {
		try {
			var ctx = fs.readFileSync(filePath);
			if (!ctx) {
				return new Buffer('');				
			}
			return ctx;
	...
	getBase64ImageExt: function(data) {
		var ext = '.png';
		var reg = new RegExp('^data:image/(\\w+);base64,', 'g');
	...
	// write base64 png
	saveImage: function(base64Data, targetPath, callback) {
		// 正常应该会处理格式，此此，暂均用png
		var data = base64Data.replace(/^data:image\/\w+;base64,/, "");
		fs.writeFile(targetPath, new Buffer(data, 'base64'), function(err) {
			if(err) {
				console.log(err);
				callback(false, err);
				return;				
