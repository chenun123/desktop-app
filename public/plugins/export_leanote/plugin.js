...
define(function() {
    ...
    var exportLeanote = {
        ...
        fixContent: function(content) {
            ...
            // strip unsafe tags
            content = resanitize.stripUnsafeTags(content, ['wbr', 'style', 'comment', 'plaintext', 'xmp', 'listing',
        ...
        findAllImages: function(note) {
            ...
            if (note.IsMarkdown) {
                ...
                // for img
                reg = new RegExp('<img([^>]*?)src=["\']?' + Api.evtService.getImageLocalUrlPrefix() + '\\?fileId=([0-9a-zA-Z]{24})["\']?(.*?)>', 'g');
                matches = reg.exec(content);
                while (matches) {
                    var all = matches[0];
